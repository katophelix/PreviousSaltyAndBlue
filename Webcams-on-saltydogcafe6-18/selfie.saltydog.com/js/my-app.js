...
jQuery(document).ready(function($){
 ...
 myApp.onPageInit('splash', function (page) {
				   ...
				   console.log(number);
				    setTimeout(function(){
 document.getElementById("newimg").src="http://saltydogcafe.com/selfie.saltydog.com/src/image-" + number + ".jpg";
    },1000);
	...
	$('iframe').attr('src','http://saltydogcafe.com/selfie.saltydog.com/selfie/dev/dl.php?image=http://saltydogcafe.com/selfie.saltydog.com/src/image-' + number + '.jpg');
...
$('.discard').on('click',function(e){
	e.preventDefault();     
mainView.router.loadPage('http://saltydogcafe.com/selfie.saltydog.com/index.php');
...
document.getElementById("photo").src="http://saltydogcafe.com/selfie.saltydog.com/src/image-" + number + ".jpg";
...
function test4(){
	...
	$.ajax({
           type: "POST",
           url: 'http://saltydogcafe.com/selfie.saltydog.com/test4.php',
...
function test6(){
	...
	$.ajax({
           type: "POST",
           url: 'http://saltydogcafe.com/selfie.saltydog.com/test6.php',
           data: $("#webshot").serialize(), // serializes the form's elements.
           success: function(data)
           {
			 console.log(data);
...
function test2(){
	var url = 'http://saltydogcafe.com/selfie.saltydog.com/test2.php';
    var postData = $('#webshot').serialize();
	var message = $('textarea[name=message]').value;
	if ($('.message').val().length === 0 ){
		$('.message').val('No Message');
	...
	$("#webshot").on('click','#submit',function(e)
{
	e.preventDefault();
	mainView.router.loadPage('http://saltydogcafe.com/selfie.saltydog.com/index.php');
    var url = 'http://saltydogcafe.com/selfie.saltydog.com/test3.php';
    var postData = $('#webshot').serialize();
    var formURL = url;
    $.ajax({
           type: "POST",
